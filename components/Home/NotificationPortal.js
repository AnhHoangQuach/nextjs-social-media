...
function NotificationPortal({ newNotification, notificationPopup, showNotificationPopup }) {
      ...
      <Segment style={{ right: '5%', position: 'fixed', top: '10%', zIndex: 1000 }}>
        ...
        <Feed>
          <Feed.Event>
            ...
            <Feed.Content>
              <Feed.Summary>
                <Feed.User onClick={() => router.push(`/${username}`)}>{name} </Feed.User> liked
                your <a onClick={() => router.push(`/post/${postId}`)}> post</a>
